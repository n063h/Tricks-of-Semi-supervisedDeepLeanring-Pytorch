...
def convLargeNoisyNet(num_classes, drop_ratio=0.0):
    return CNN(CNN_block, [3,3,3], num_classes, drop_ratio),CNN(CNN_block, [3,3,3], 2, drop_ratio)

def test():
    print('--- run conv_large test ---')
    x = torch.randn(2,3,32,32)
    for net in [convLargeNoisyNet(10)]:
        print(net)
        y = net(x)
        print(y.size())
