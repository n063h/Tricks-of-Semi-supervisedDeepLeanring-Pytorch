...
class CNN(nn.Module):
    ...
    def __init__(self, block, num_blocks, num_classes=10, drop_ratio=0.0):
        ...
        self.fc2 = nn.Linear(self.out_plane, 64)
    ...
    def forward(self, x):
        ...
        return self.fc1(out),self.fc3(out2)


def convLargeNoisyHead2(num_classes, drop_ratio=0.0):
    return CNN(CNN_block, [3,3,3], num_classes, drop_ratio)

def test():
    print('--- run conv_large test ---')
    x = torch.randn(2,3,32,32)
    for net in [convLargeNoisyHead2(10)]:
        print(net)
        y = net(x)
        print(y.size())
