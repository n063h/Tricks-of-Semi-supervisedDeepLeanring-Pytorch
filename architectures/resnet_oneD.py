from architectures.context_block import ContextBlock
...
class ResNet_d(nn.Module):
    ...
    def forward(self, x):
        x = x.view([int(x.size(dim=0)), 1, 228])
        x = self.conv1(x)
        x = self.bn1(x)
        x = self.relu(x)
...
def Net(num_cls=50,dropout=0):
    return get_model(4, 61, 3)
#
# def get_model(M, L, C):
#     model_18 = ResNet_c(
