...
class Trainer:
    ...
    def __init__(self, model, ema_model, optimizer, device, config):
        ...
        self.global_step = 0
        self.epoch       = 0
        self.mask_noisy  = config.mask_noisy
    ...
    def train_iteration(self, label_loader, unlab_loader, print_freq):
        ...
        for ((x1,_), label_y), ((wu,su), unlab_y) in zip(cycle(label_loader), unlab_loader):
            ...
            ##=== forward ===
            outputs,o_notnoisy = self.model(label_x)
            loss = self.lce_loss(outputs, label_y)
            loss_notnoisy=self.lce_loss(o_notnoisy, torch.zeros_like(label_y).to(self.device))
            loss+=loss_notnoisy
            loop_info['lloss'].append(loss.item())
            loop_info['lnoisyloss'].append(loss_notnoisy.item())
            ...
            with torch.no_grad():
                woutputs,wo_notnoisy = self.model(weak_u)
                woutputs = F.softmax(woutputs, 1)
                wprobs, wpslab = woutputs.max(1)
                wo_notnoisy_prob=F.softmax(wo_notnoisy, 1)
                w_notnoisy_probs,w_notnoisy_lab=wo_notnoisy_prob.max(1)
            ## cross-entropy loss for confident unlabeled data
            mask = wprobs.ge(self.threshold).float()
            noisy_mask=torch.ones_like(mask)
            
            noisy_mask = w_notnoisy_probs.lt(self.mask_noisy/self.rampup(self.epoch)).float()
            mask*=noisy_mask
            # label loss ignore with noisy and prob masks
            soutputs,so_noisy = self.model(strong_u)
            uloss  = torch.mean(mask* self.uce_loss(soutputs, wpslab))
            uloss_notnoisy=self.uce_loss(wo_notnoisy, torch.zeros_like(wpslab).to(self.device))
            uloss_noisy=self.uce_loss(so_noisy, torch.ones_like(wpslab).to(self.device))
            # noisy loss ignore with noisy mask
            uloss_noisy_all=torch.mean(noisy_mask* (uloss_noisy+uloss_notnoisy))
            uloss+=uloss_noisy_all
            uloss *= self.usp_weight
            loss  += uloss
            loop_info['uloss'].append(uloss.item())
            loop_info['uloss_noisy_all'].append(uloss_noisy_all.item())
        ...
        print(f">>>[train]", self.gen_info(loop_info, label_n, unlab_n, False),flush=True)
    ...
    def test_iteration(self, data_loader, print_freq):
        ...
        for batch_idx, (data, targets) in enumerate(data_loader):
            ...
            ema_outputs,_ = self.ema_model(data)
        ...
        print(f">>>[test]", self.gen_info(loop_info, label_n, unlab_n, False),flush=True)
        return loop_info, label_n
