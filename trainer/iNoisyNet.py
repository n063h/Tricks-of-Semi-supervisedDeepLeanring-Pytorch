...
class Trainer:
    ...
    def train_iteration(self, label_loader, unlab_loader, print_freq):
        ...
        for ((x1,_), label_y), ((wu,su), unlab_y) in zip(cycle(label_loader), unlab_loader):
            ...
            uloss_notnoisy=self.lce_loss(wo_notnoisy, torch.zeros_like(wpslab).to(self.device))
            uloss_noisy=self.lce_loss(so_noisy, torch.ones_like(wpslab).to(self.device))
            uloss_noisy_all=uloss_noisy+uloss_notnoisy
